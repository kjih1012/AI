...
n_classes = 5 # 결과값으로 분류할 가짓 수(우리는 총 10가지 음향을 분류함)
...
saver.restore(sess, "./mytotalmodel.ckpt")
...
def upload_file():
    if request.method == 'POST':
        ...
        if file :
            ...
            ans_p1 = np.zeros(shape=[5], dtype=np.float32)
            ans_p2 = np.zeros(shape=[5], dtype=np.float32)
            ans_1sec = np.zeros(shape=[5], dtype=np.float32)
            ans_fin = np.zeros(shape=[5], dtype=np.float32)
            #
